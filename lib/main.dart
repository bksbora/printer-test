...
import 'package:printer/phomemo.dart';
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  PhomemoPrinter printer = PhomemoPrinter.m110;
  ...
  @override
  void initState() {
    bluetooth = Bluetooth(onUpdate: _bleUpdate, names: ['m110', 'm120', 'm220', 'Q199E4CU7260973']

        /// 'Q--------------' Place the name of the m-type (m110,m120,m220) printer here. This is displayed on the front of the printer.
        );
  ...
  Future<void> printPhomemo({Uint8List? image, String? name, Size? size}) async {
    ...
    await label.printLabel([letter], printer: printer, spacing: 5, labelSize: labelSize, rotate: !PhomemoPrinter.isMType(printer) // do not rotate the image if using the m-type printer
        ).then((value) {
      printing = false;
    });
  }
  ...
  Future<img.Image?> textToImage(String? text, Size size) async {
    img.Image? letter = text != null
        ? await PhomemoHelper.generateImageFromText(
            TextSpan(
              text: text,
              style: const TextStyle(fontFamily: 'MuseoSans', fontSize: 34, color: Colors.black),
            ),
            size: size * 8,
          )
        : null;
            ...
            showType == 'Custom'
                ? Container(
                    ...
                    child: TextField(
                      ...
                      onChanged: (t) async {
                        image = img.encodePng((await textToImage(textController.text, labelSize))!);
                        setState(() {});
                      },
              ...
              children: [
                ...
                Container(
                    ...
                    child: TextField(
                      ...
                      onChanged: (t) {
                        labelSize =
                            Size(sizeController[0].text != '' ? double.parse(sizeController[0].text) : double.infinity, sizeController[1].text != '' ? double.parse(sizeController[1].text) : 12);
                        setState(() {});
                      },
                      decoration: const InputDecoration(
                          isDense: true,
            ...
            if (showType == 'Custom')
              Container(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(10), image: image == null ? null : DecorationImage(image: MemoryImage(image!))),
                  alignment: Alignment.centerLeft,
                ),
              )
          ],
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          if (bluetooth.deviceConnected()) {
            ...
            if (((Platform.isMacOS || Platform.isIOS) && await Permission.bluetooth.request().isGranted) ||
