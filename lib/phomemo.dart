...
class Phomemo {
  Phomemo({required this.send, this.packetSize = 128});
  ...
  Future<void> printLabel(List<img.Image?> src, {required PhomemoPrinter printer, Size labelSize = const Size(12, double.infinity), int? spacing, bool rotate = true}) async {
    if (rotate) {
...
class PhomemoHelper {
  ...
  static Future<img.Image?> generateNameTag(
    ...
    m.TextPainter textPainter = m.TextPainter(text: text, textAlign: TextAlign.center, textDirection: TextDirection.ltr)
  ...
  static Future<img.Image?> generateImage(m.TextSpan text, {required Size size, int padding = 0}) async {
    return generateImageFromText(text, size: size, padding: padding);
  }

  /// Generate Image from Text in the corrected lable size format.
  static Future<img.Image?> generateImageFromText(m.TextSpan text, {required Size size, int padding = 0}) async {
    m.TextPainter textPainter = m.TextPainter(text: text, textAlign: TextAlign.center, textDirection: TextDirection.ltr)..layout(minWidth: 0, maxWidth: double.infinity);
    ...
    if (textPainter.width > size.width || size.width == double.infinity) {
      size = Size(textPainter.width + padding, size.width == double.infinity ? size.height : (textPainter.width + padding) * ratio);
    }
    ...
    final Picture picture = recorder.endRecording();
    final res = await picture.toImage(size.width == double.infinity ? (textPainter.width).toInt() + padding : size.width.toInt() + padding,
